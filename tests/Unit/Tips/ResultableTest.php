...
class ResultableTest extends \Tests\TestCase
{
    public function testStatisticResult()
    {
        ...
        $result->doThresholdComparison(0.5, \App\Tips\TipCoupledStatistic::COMPARISON_OPERATOR_GREATER_THAN);
        $this->assertEquals('1', $result->getResultString());
        $this->assertTrue($result->hasPassed());
        $this->assertEquals('SomeEntity', $result->getName());

        $result->doThresholdComparison(0.5, \App\Tips\TipCoupledStatistic::COMPARISON_OPERATOR_LESS_THAN);
    ...
    public function testStatisticResultCollection()
    {
        ...
        $collection->doThresholdComparison(0.5, \App\Tips\TipCoupledStatistic::COMPARISON_OPERATOR_GREATER_THAN);
