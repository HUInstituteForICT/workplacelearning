    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <ul>
                    <li>{{ Lang::get('elements.calendar.labels.date') }}/{{ Lang::get('activity.feedback.time') }}: {{ date('d-m', strtotime($lap->date)) }}, {{ Lang::get('activity.feedback.duration') }}
                    ...
                    <li>{{ (is_null($lap->res_person_id)) ? Lang::get('activity.feedback.not-used-source') : Lang::get('activity.feedback.used-source') }} ({{ $lap->getResourceDetail() }})
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="row well">
            ...
            <div class="col-md-2 form-group buttons">
                ...
                <label><input type="radio" name="notfinished"
                              value="Geen/Weinig Ervaring" {{ ($fb->isSaved()) ? (($fb->notfinished == "Geen/Weinig Ervaring") ? "checked" : "") : "checked" }}/><span>{{ Lang::get('activity.none') }}/{{ Lang::get('activity.feedback.little-experience') }}</span></label>
                <label><input type="radio" name="notfinished"
                ...
                <label class="expand-click"><input type="radio" name="notfinished" value="Anders" {{
                    ...
                    }}/><span class="new">{{ Lang::get('activity.other') }} ({{ Lang::get('activity.add') }})</span></label>
                    <input {!! ($fb->isSaved()) ? "disabled " : "" !!}class="cond-hidden" type="text" name="newnotfinished" placeholder="{{ Lang::get('activity.description') }}"
                           maxlength="80"
                            {!!
                ...
                <div class="col-md-2 form-group buttons">
                    ...
                    <label><input type="radio" name="support_requested" value="0" {{ ($fb->isSaved()) ? (($fb->support_requested == "0") ? "checked   " : "") : "checked" }}/><span>{{ Lang::get('activity.feedback.no-help') }}</span></label>
                    <label><input type="radio" name="support_requested" value="1" {{ ($fb->isSaved() && $fb->support_requested == "1") ? "checked" : "" }}/><span>{{ Lang::get('activity.feedback.help-received') }}</span></label>
                    <label><input type="radio" name="support_requested" value="2" {{ ($fb->isSaved() && $fb->support_requested == "2") ? "checked" : "" }}/><span>{{ Lang::get('activity.feedback.help-asked') }}</span></label>
                </div>
                <div id="expand-toggle" class="col-md-3 form-group">
                    ...
                    <textarea class="form-control fit-bs" {!! ($fb->isSaved()) ? "disabled " : "" !!}name="supported_provided_wp"
                              maxlength="150"
                              placeholder="{{ Lang::get('activity.feedback.help-received-example') }}" rows="8" cols="40">{{ ($fb->isSaved()) ? $fb->supported_provided_wp : "" }}</textarea>
                </div>
                <div class="col-md-2 form-group buttons">
                    <h4>{{ Lang::get('activity.feedback.happy-with-progress') }}</h4>
                    <label><input type="radio" name="progress_satisfied" value="2" {{ ($fb->isSaved()) ? (($fb->progress_satisfied == "2") ? "checked" : "") : "checked" }}/><span>{{ Lang::get('general.yes') }}</span></label>
                    <label><input type="radio" name="progress_satisfied" value="1" {{ ($fb->isSaved() && $fb->progress_satisfied == "1") ? "checked" : "" }}/><span>{{ Lang::get('general.no') }}</span></label>
                </div>
                <div class="col-md-3 form-group">
                    ...
                    <textarea class="form-control fit-bs" {!! ($fb->isSaved()) ? "disabled " : "" !!}name="initiatief"
                              maxlength="150"
                              placeholder="{{ Lang::get('activity.feedback.initiative-explanation') }}" rows="8" cols="40" >{{ ($fb->isSaved()) ? $fb->initiative : "" }}</textarea>
                </div>
            ...
            <div class="form-horizontal well" style="min-height: 350px;">
                ...
                <div class="col-sm-4 form-group">
                    ...
                    <textarea class="form-control fit-bs" {!! ($fb->isSaved()) ? "disabled " : "" !!}name="vervolgstap_zelf"
                              maxlength="150"
                              placeholder="{{ Lang::get('activity.feedback.which-person') }}" rows="8" cols="40">{{ ($fb->isSaved()) ? $fb->nextstep_self : "" }}</textarea>
                </div>
                <div class="col-sm-4 form-group">
                    ...
                    <textarea class="form-control fit-bs" {!! ($fb->isSaved()) ? "disabled " : "" !!}name="ondersteuning_werkplek"
                              maxlength="150"
                              placeholder="{{ Lang::get('activity.feedback.help-needed-example') }}" rows="8" cols="40">{{ ($fb->isSaved()) ? $fb->support_needed_wp : "" }}</textarea>
                    <br /><input type="checkbox" name="ondersteuningWerkplek" value="Geen" {{ ($fb->isSaved() && $fb->support_needed_wp == "Geen") ? "checked" : "" }}/> {{ Lang::get('activity.feedback.no-help-needed') }}
                    <br style="clear: both;" />
                </div>
                <div class="col-sm-4 form-group">
                    ...
                    <textarea class="form-control fit-bs" {!! ($fb->isSaved()) ? "disabled " : "" !!}name="ondersteuning_opleiding"
                              maxlength="150"
                              placeholder="{{ Lang::get('activity.feedback.help-school-needed-example') }}" rows="8" cols="40">{{ ($fb->isSaved()) ? $fb->support_needed_ed : "" }}</textarea>
                    <br /><input type="checkbox" name="ondersteuningOpleiding" value="Geen" {{ ($fb->isSaved() && $fb->support_needed_ed == "Geen") ? "checked" : "" }}/> {{ Lang::get('activity.feedback.no-school-help-needed') }}
                    <br style="clear: both;" />
