        ...
        {{ Form::open(array('url' => route('process-producing-update', ['id' => $activity->lap_id]), 'class' => 'form-horizontal')) }}
            <div class="row well">
                <div class="col-md-2 form-group">
                    ...
                    <textarea class="form-control fit-bs" name="{{ Lang::get('activity.description') }}" required maxlength="80" rows="5" cols="19">{{ (count($errors) > 0) ? old('omschrijving') : $activity->description }}</textarea>
                </div>
                ...
                <div class="col-md-2 form-group buttons">
                    ...
                    <div id="swvcontainer">
                        <label class="expand-click"><input type="radio" name="resource" value="persoon" {{ (old('resource') == 'persoon') ? 'checked' : ($activity->res_person_id != null ) ? 'checked' : null }} /><span>{{ Lang::get('activity.person') }}</span></label>
                    ...
                    <div id="solocontainer">
                        <label class="expand-click"><input type="radio" name="resource" value="alleen" {{ (old('resource') == 'alleen') ? 'checked' : ($activity->res_person_id == null && $activity->res_material_id == null) ? 'checked' : null }} /><span>{{ Lang::get('activity.alone') }}</span></label>
                    </div>
                ...
                <div class="col-md-2 form-group buttons">
                    ...
                    <label><input type="radio" name="status" value="1" {{ (old('status') == 1) ? 'checked' : ($activity->status_id == 1) ? 'checked' : null }} /><span>{{ Lang::get('activity.finished') }}</span></label>
                    <label><input type="radio" name="status" value="2" {{ (old('status') == 2) ? 'checked' : ($activity->status_id == 2) ? 'checked' : null }} /><span>{{ Lang::get('activity.busy') }}</span></label>
                    <label><input type="radio" name="status" value="3" {{ (old('status') == 3) ? 'checked' : ($activity->status_id == 3) ? 'checked' : null }} /><span>{{ Lang::get('activity.transferred') }}</span></label>
                </div>
