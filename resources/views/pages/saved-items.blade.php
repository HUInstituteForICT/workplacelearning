...
@section('content')
    <?php
    use App\SavedLearningItem;use App\Student;
    ...
    <div class="container-fluid">
        ...
        <div class="row">
                ...
                @foreach($sli as $item)
                    @if($item->category === 'tip' && $item->folder === null)
                        ...
                        <div class="alert"
                             ...
                             role="alert">
                            <a href="{{ route('saved-learning-items-delete', ['sli' => $item])}}"
                               onclick="return confirm('{{ __('saved_learning_items.delete-confirmation') }}')"><span
            ...
            <div class="col-md-6">
                @card
                <h2 class="maps">{{ __('folder.folders') }}</h2> <a data-target="#addFolderModel"
                                                                    data-toggle="modal"><span
                            class="glyphicon glyphicon-plus add-collapse" aria-hidden="true"></span></a>
                            ...
                            <div id="{{$folder->folder_id}}" class="panel-collapse collapse">
                                ...
                                @if (count($folder->folderComments))
                                    ...
                                    <section class="section comment-section">
                                        <h5>{{ __('folder.comments') }} <span
                                                    class="badge">{{ count($folder->folderComments)}}</span></h5>
                                        @foreach ($folder->folderComments as $comment)
                                            @if ($comment->author->isStudent())
                                                <div class="comment student-comment">
                                                    <p class="sub-title-light"><span class="glyphicon glyphicon-time"
                                                                                     aria-hidden="true"></span> {{date('H:i', strtotime($comment->created_at))}}
                                                    </p>
                                ...
                                @if ($folder->isShared())
                                    <div class="panel-footer">
                                        ...
                                        <div class="form-group">
                                            <input type='text' value="{{$folder->folder_id}}" name='folder_id'
                                                   class="form-control folder_id">
                                        </div>
                                        <div class="form-group">
                                ...
                                @elseif (!$folder->isShared() && $student->getCurrentWorkplaceLearningPeriod()->hasTeacher())
                                    <div class="panel-footer">
                                        ...
                                        <div class="form-group">
                                            <textarea placeholder="{{ __('folder.question') }}" name='folder_comment'
                                                      class="form-control folder_comment" maxlength="255"
                                                      required></textarea>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select name="teacher" class="form-control">
                                                <?php /** @var $wplp \App\WorkplaceLearningPeriod */ ?>
                                ...
                                @else
                                    <div class="panel-footer">
                                        <div class="custom-alert alert alert-info"
                                             role="alert">{{ __('folder.no-teacher') }}</div>
                                    </div>
                                @endif
                            </div>
                        </div>
