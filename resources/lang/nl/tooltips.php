<?php

return array(
  'producing_category' => 'Kies hieronder in welke categorie de activiteit die je toe wilt voegen het beste past. Als geen enkele categorie geschikt lijkt, kun je zelf een categorie toevoegen met de knop “Anders (Toevoegen)”.',
  'producing_with' => 'Werken en leren doe je soms alleen, en soms juist niet. Hier kun je aangeven welke personen je als hulpbronnen hebt ingeschakeld. Later kun je dan analyseren met wie je veel samenwerkt, of juist niet.',
  'producing_theory' => 'Om complexe problemen op te lossen, moet je soms de theorie er weer even bij pakken of moet je zaken (grondig) uitzoeken. Hier kun je aangeven welke bronnen je hebt geraadpleegd. Later kun je dit dan weer terugvinden, wat je natuurlijk goed kan helpen bij het schrijven van je stageverslag.',
  'producing_status' => 'Hier kun je aangeven of de activiteit die je invoert al afgerond is, of dat je er nog mee bezig bent. In het laatste geval kun je vervolgactiviteiten koppelen aan deze activiteit en dit later ook weer terugvinden als keten van activiteiten.',
  'producing_difficulty' => 'Hier kun je aangeven hoe moeilijk je deze activiteit vond. Op de Analyse-pagina kun je dan analyseren welk type activiteiten je het moeilijkst of juist het makkelijkst vindt.',
  'producing_feedback' => 'Hier kun je ervoor kiezen om jezelf feedback te geven deze activiteit, bijvoorbeeld wat er nou precies lastig aan was. Dit kun je later teruglezen en gebruiken voor bijvoorbeeld je eindverslag.',
  'acting_when' => 'Kies hier de categorie die het leermoment het best beschrijft. Het is ook mogelijk om een nieuwe categorie te creëren.',
  'acting_with' => 'Werken en leren doe je soms alleen, en soms juist niet. Hier kun je aangeven welke personen je als hulpbronnen hebt ingeschakeld. Later kun je dan analyseren met wie je veel samenwerkt, of juist niet.',
  'acting_theory' => 'Om complexe problemen op te lossen, moet je soms de theorie er weer even bij pakken of moet je zaken (grondig) uitzoeken. Hier kun je aangeven welke bronnen je hebt geraadpleegd. Later kun je dit dan weer terugvinden, wat je natuurlijk goed kan gebruiken bij het schrijven van je leerwerkverslag.',
  'acting_learned' => 'Geef hier helder aan wat je precies hebt geleerd. Wat betekent dit voor jouw handelen in de toekomst? Wat ga je de volgende keer anders doen, of juist hetzelfde?',
  'acting_required_wp' => 'Het kan zijn dat je ondersteuning nodig hebt van jouw werkplek om de volgende stap te zetten na dit leermoment. Beschrijf hier welke ondersteuning je graag zou ontvangen.',
  'acting_required_ep' => 'Het kan zijn dat je ondersteuning nodig hebt vanuit de HU om de volgende stap te zetten na dit leermoment. Beschrijf hier welke ondersteuning je graag zou ontvangen.',
  'acting_learninggoal' => 'Hier kun je dit leermoment koppelen aan een van je leervragen of een leermoment typeren als een ongepland leermoment.',
  'acting_competence' => 'Hier kun je dit leermoment koppelen aan een van de competenties van jouw opleiding. Je moet hier kiezen voor die competentie die het best past bij dit leermoment. In het leerwerkverslag kun je uiteraard nog andere competenties toevoegen. Voor een overzicht van de competenties, subschalen en indicatoren, zie de link hieronder.',
);
